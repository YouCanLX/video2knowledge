...
- ✅ 联合索引合集内的全部 Markdown 笔记，通过已配置的大模型将原有来源/YAML 标签替换为兼容
  Obsidian 的语义层级标签，同时保留其他 frontmatter；标签生成要求 3–6 个简洁、规范的概念，
  过滤颜色值、纯数字及不完整片段，并在整个知识库内保守归并近似重复的主题标签。单篇处理失败时
  原 Markdown 保持不变。知识管理界面支持搜索、合集摘要、全局标签树、文档—标签关系图，以及
  合集卡片折叠、多选合集后定向调用大模型生成标签，并支持高频标签和任意标签树节点的一键筛选；
  还可调用大模型把出现频率最高的 100 个主题标签整理为可展开的“领域 → 方向 → 标签”图谱，
  在叶节点展示关联笔记，并把 `knowledge/领域/方向/标签` 路径同步回 Markdown。
...
旧数据迁移除了数据库路径，还会依次使用唯一视频 ID、规范化文件名和内嵌元数据补充匹配。仍无法安全确定归属的文件会保存在 `library/.unmatched-media/`，随后删除已经清空的旧 `media/` 目录。
