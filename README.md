...
- ✅ Jointly indexes every Markdown note inside a collection and uses the configured LLM to
  ...
  filters for popular tags and every tag-tree node. It can also use the LLM to organize the 100
  most frequent topic tags into an expandable domain → direction → tag graph, display the linked
  notes at every leaf, and synchronize `knowledge/domain/direction/tag` paths back to Markdown.
